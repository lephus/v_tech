...
class AcceptedView extends GetView<TimeKeepingController>{
  @override
          ...
          children: [
            Row(
              children: <Widget>[
                ...
                IconButton(onPressed: (){controller.GetAcceptedList();}, icon: Icon(Icons.search))
              ],
            ),
            Expanded(
                child: Obx(()=>
                controller.idLoadAcceptedList.value?
                Center(child:CircularProgressIndicator()):
                controller.AcceptedList.length == 0?
                Center(child: Text("Không có dữ liệu")):
                Scrollbar(
                  child: RefreshingView(
                      controller: controller.refreshControllerAccepted,
                      onRefresh: controller.onRefreshAccepted,
                      onLoadmore: controller.onLoadMoreAccepted,
                      child:
                      ListView.builder(
                          shrinkWrap: true,
                          itemCount: controller.AcceptedList.length,
                          itemBuilder: (context, index) {
                            TimeKeepingModel item = controller.AcceptedList[index];
                            return StickyHeader(
                              header:
                              Container(
                                width: 0.0,
  ...
  void _showDatePicker(BuildContext context, bool isStart) async {
    DatePicker.showDatePicker(context,
        ...
        onChanged: (date) {
          ...
          isStart
              ? controller.fromAcceptedList.value = getDateFilter(date)
              : controller.toAcceptedList.value = getDateFilter(date.add(Duration(days: 1)));
          ...
          Expanded(
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                child: Row(
                  children: <Widget>[
                    Obx(()=>Text('${controller.fromAcceptedList.value}')),
                    Spacer(),
      ...
      child: Row(
        children: <Widget>[
          Expanded(
            flex: 13,
            child: GestureDetector(
              onTap: () => controller.fromAcceptedList.value == 'Ngày bắt đầu' ||
                  controller.fromAcceptedList.value == ''
                  ? null
              ...
              child: Container(
                ...
                child: Row(
                  children: <Widget>[
                    Obx(()=>Text('${controller.toAcceptedList.value}',
                      style: TextStyle(
                          color: controller.fromAcceptedList.value == 'Ngày bắt đầu' ||
                              controller.fromAcceptedList.value == ''
                              ? Colors.grey
                              : Colors.black),
                    )),

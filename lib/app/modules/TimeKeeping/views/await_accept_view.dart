...
class AwaitAcceptView extends GetView<TimeKeepingController>{
  @override
          ...
          children: [
            Row(
              children: <Widget>[
                ...
                IconButton(onPressed: (){controller.GetAwaitAcceptList();}, icon: Icon(Icons.search))
              ],
            ),
            Expanded(
                child: Obx(()=>
                controller.idLoadAwaitAcceptList.value?
                Center(child:CircularProgressIndicator()):
                controller.AwaitAcceptList.length == 0?
                Center(child: Text("Không có dữ liệu")):
                Scrollbar(
                  child: RefreshingView(
                      controller: controller.refreshControllerAwaitAccept,
                      onRefresh: controller.onRefreshAwaitAccept,
                      onLoadmore: controller.onLoadMoreAwaitAccept,
                      child:
                      ListView.builder(
                          shrinkWrap: true,
                          itemCount: controller.AwaitAcceptList.length,
                          itemBuilder: (context, index) {
                            TimeKeepingModel item = controller.AwaitAcceptList[index];
                            return StickyHeader(
                              header:
                              Container(
                                width: 0.0,
  ...
  void _showDatePicker(BuildContext context, bool isStart) async {
    DatePicker.showDatePicker(context,
        ...
        onChanged: (date) {
          ...
          isStart
              ? controller.fromAwaitAcceptList.value = getDateFilter(date)
              : controller.toAwaitAcceptList.value = getDateFilter(date.add(Duration(days: 1)));
          ...
          Expanded(
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                child: Row(
                  children: <Widget>[
                    Obx(()=>Text('${controller.fromAwaitAcceptList.value}')),
                    Spacer(),
      ...
      child: Row(
        children: <Widget>[
          Expanded(
            flex: 13,
            child: GestureDetector(
              onTap: () => controller.fromAwaitAcceptList.value == 'Ngày bắt đầu' ||
                  controller.fromAwaitAcceptList.value == ''
                  ? null
              ...
              child: Container(
                ...
                child: Row(
                  children: <Widget>[
                    Obx(()=>Text('${controller.toAwaitAcceptList.value}',
                      style: TextStyle(
                          color: controller.fromAwaitAcceptList.value == 'Ngày bắt đầu' ||
                              controller.fromAwaitAcceptList.value == ''
                              ? Colors.grey
                              : Colors.black),
                    )),
